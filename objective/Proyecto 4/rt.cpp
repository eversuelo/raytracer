...
Vector MEsfera(void)
{
	double xiphi = ((double)rand() / (RAND_MAX)); //((double)rand() / (RAND_MAX));
	double xiteta = ((double)rand() / (RAND_MAX));
...
{
	double xiphi = ((double)rand() / (RAND_MAX));
	double xiteta = ((double)rand() / (RAND_MAX)); //((double)rand() / (RAND_MAX));
...
Color BRDF(Vector Wo, Vector Wh, Vector Wi, Vector n, int id)
{
	if (spheres[id].eta.x != 0)
	{
		...
		return F * (Gval * Dval) * (1 / denominador);
	}
	else
	{
...
Vector MWh(double alfa)
{
	...
	double xiphi = ((double)rand() / (RAND_MAX));	//((double)rand() / (RAND_MAX));
	double xitheta = ((double)rand() / (RAND_MAX)); //((double)rand() / (RAND_MAX));
...
Vector CosHemisphere(Vector n, Point x, int id, Vector Wo)
{
	...
	if (spheres[id].eta.x != 0)
	{
		...
		double invP = (1 / PBRDF(Wh, Wo, id));
		//double invP = (1 / PCosHemisferico(costheta));
		temp = (fr.mult(Le) * costheta) * invP;
		return temp;
	}
	else
	{
...
Color AnguloSolido(Vector n, Point x, int FuenteLuminosa, int id, Vector Wo)
{
	Vector s;
	Vector t;
...
Color FuenteDeArea(Vector n, Point x, int FuenteLuminosa, int id, Vector Wo)
{
	// Para Fuente de Area
	Ray rayo = Ray(spheres[FuenteLuminosa].p, MEsfera());
	double tiempo = spheres[FuenteLuminosa].intersect(rayo);
	Point xprima = rayo.d * tiempo + rayo.o;
	Vector normal = xprima - spheres[FuenteLuminosa].p; //
	Vector direction = xprima - x;						//Dirección desde el punto que queremos iluminar hacia la fuente luminosa.
	double distancia;
	direction.normalize();
	Color Le = ComputeLe(Ray(xprima, direction * -1), x, id, distancia, FuenteLuminosa);
	normal.normalize();
	double costhetaI = n.dot(direction);
	double costhetaO = normal.dot(direction * -1);
	Vector s, t;
	coordinateSystem(n, s, t);
	Vector Wi = GlobalToLocal(n, s, t, direction);
	Vector Wh = (Wo + Wi) * (1 / (Wo + Wi).magnitude());
	Color fr = BRDF(Wo, Wh, Wi, n, id);
	Vector color = (Le.mult(fr) * costhetaI) * (1 / PFuenteDeArea(costhetaO, distancia, FuenteLuminosa));
	return color;
}
...
Color EstimadorMonteCarlo(Ray ray, Vector n, Point x, int id, int opcion)
{
	Color temp = Color();
	Vector s;
	Vector t;
	coordinateSystem(n, s, t);
	Vector Wo = GlobalToLocal(n, s, t, ray.d * -1);
		...
		for (int FuenteLuminosa = 0; NoSpheres > FuenteLuminosa; FuenteLuminosa++)
		{
			if ((spheres[FuenteLuminosa].ke.x > 0 || spheres[FuenteLuminosa].ke.y > 0 || spheres[FuenteLuminosa].ke.z > 0))
			{
				if (spheres[FuenteLuminosa].r > 0)
				{
					if (opcion == 1)
					{
						temp = temp + AnguloSolido(n, x, FuenteLuminosa, id, Wo);
					}
					else if (opcion == 2)
					{ //Fuente de Area
						temp = temp + FuenteDeArea(n, x, FuenteLuminosa, id, Wo);
					}
				}
				else
				{
					temp = temp + FuentePuntual(n, x, FuenteLuminosa, id, Wo);
				}
			}
		}
	}
	...
	{
		return CosHemisphere(n, x, id, Wo);
	}
...
Color shade(const Ray &r, int opcion)
{
	double t;
	int id = 0;
	...
	{ // determinar el color que se regresara
		...
		Color radiancia = EstimadorMonteCarlo(r, n, x, id, opcion);
		return radiancia;
	}
}
int interface()
...
#pragma omp parallel for schedule(dynamic, 1)
	for (int y = 0; y < h; y++)
	{
		...
		for (int x = 0; x < w; x++)
		{
			...
			{
				double dx = ((float)rand() / RAND_MAX) - 0.5, dy = ((float)rand() / RAND_MAX) - 0.5;
