#!/usr/bin/env bash
# Corre SOLO la fase 06 (Tema 7: medios participantes — estado del arte) con la
...
#
# A diferencia del curso, la base NO es sdd/base: la fase 6 parte del resultado
...
S="${ROOT}/start/sdd/phase-06/spec.md"
grep -q "status: approved" "${S}" || die "spec de fase 6 no aprobada"
...
GATE_CMD="make && bash sdd/phase-06/check.sh"
...
GATE="fail"
set +e
( cd start && make && bash sdd/phase-06/check.sh ) </dev/null >"${LOG%.log}.gate.log" 2>&1 && GATE="ok"
set -e
echo "→ fase 6: run=${RUN_STATUS} gate=${GATE} (${DUR_S}s, run_id=${RUN_ID:-—})"
...
git add start/rt.cpp start/CLAUDE.md start/.claude start/sdd/phase-06 data/metricas.csv RESUMEN-METRICAS.md 2>/dev/null || true
...
git commit -q -m "tema7 ${CELL}: fase 6 run=${RUN_STATUS} gate=${GATE} (${DUR_S}s, run ${RUN_ID:-sin-id})" || true
git tag -f "celda/${CELL_TAG}/f6" >/dev/null 2>&1 || true
echo "→ fase Tema 7 terminada: gate=${GATE} · evidencia en data/runs/fase6-* · tag celda/${CELL_TAG}/f6"
