...
// Fase 04-05: Escenas con materiales microfacet para conductores
...
// Conductor aluminio con α=0.03 para fase 5b multil
ConductorMaterial aluminum_rough(0.03,
	Color(1.66058, 0.88143, 0.521467),
	Color(9.2282, 6.27077, 4.83803));
...
// Escena "ptexp_areal": una esfera emisora para phase 5b
Sphere scene_ptexp_areal[] = {
	Sphere(1e5,  Point(-1e5 - 49, 0, 0),   &wall_left,     Color()), // pared izq
	Sphere(1e5,  Point(1e5 + 49, 0, 0),    &wall_right,    Color()), // pared der
	Sphere(1e5,  Point(0, 0, -1e5 - 81.6), &wall_back,     Color()), // pared detras
	Sphere(1e5,  Point(0, -1e5 - 40.8, 0), &wall_floor,    Color()), // suelo
...
const int scene_ptexp_areal_count = 8;

// Escena "ptexp_point": una puntual para phase 5b (I=4000)
Sphere scene_ptexp_point[] = {
	Sphere(1e5,  Point(-1e5 - 49, 0, 0),   &wall_left,     Color()), // pared izq
	Sphere(1e5,  Point(1e5 + 49, 0, 0),    &wall_right,    Color()), // pared der
	Sphere(1e5,  Point(0, 0, -1e5 - 81.6), &wall_back,     Color()), // pared detras
	Sphere(1e5,  Point(0, -1e5 - 40.8, 0), &wall_floor,    Color()), // suelo
	...
	Sphere(16.5, Point(-23, -24.3, -34.6), &aluminum,      Color()), // conductor izq
	Sphere(16.5, Point(23, -24.3, -3.6),   &gold,          Color()), // conductor der
	Sphere(0.0,  Point(0, 24.3, 0),        &white_diffuse, Color(4000, 4000, 4000)) // puntual
};
const int scene_ptexp_point_count = 8;

// Escena "ptexp_multil": 2a1p con aluminio α=0.03 para phase 5b
Sphere scene_ptexp_multil[] = {
	Sphere(1e5,  Point(-1e5 - 49, 0, 0),   &wall_left,     Color()), // pared izq
	Sphere(1e5,  Point(1e5 + 49, 0, 0),    &wall_right,    Color()), // pared der
	Sphere(1e5,  Point(0, 0, -1e5 - 81.6), &wall_back,     Color()), // pared detras
	Sphere(1e5,  Point(0, -1e5 - 40.8, 0), &wall_floor,    Color()), // suelo
	Sphere(1e5,  Point(0, 1e5 + 40.8, 0),  &wall_ceil,     Color()), // techo
	Sphere(16.5, Point(-23, -24.3, -34.6), &aluminum_rough, Color()), // conductor izq con α=0.03
	Sphere(16.5, Point(23, -24.3, -3.6),   &gold,          Color()), // conductor der
	Sphere(0.0,  Point(0, 24.3, 0),        &white_diffuse, Color(2000, 2000, 2000)), // puntual
	Sphere(10.5, Point(-23, 24.3, 0),      &white_diffuse, Color(12, 5, 5)),        // emisor 1
	Sphere(5.0,  Point(23, 24.3, -50),     &white_diffuse, Color(5, 5, 12))         // emisor 2
};
const int scene_ptexp_multil_count = 10;
...
int emitters_ptexp_areal[] = {7};
int emitters_ptexp_areal_count = 1;
int emitters_ptexp_point[] = {7};
int emitters_ptexp_point_count = 1;
int emitters_ptexp_multil[] = {7, 8, 9};
int emitters_ptexp_multil_count = 3;
...
int spp = 32;
const char *current_mode = "issa"; // "issa", "isarea", "isbrdf", "mis", "ptexp"
int ptexp_max_bounce = 10;
...
inline bool is_finite_pdf(double pdf) {
	return pdf > 1e-6 && pdf < 1e20;
}

inline double mis_weight(double p_a, double p_b, double beta = 2.0) {
	if (p_a < 1e-6) return 0.0;
	double p_a_beta = pow(p_a, beta);
	double p_b_beta = pow(p_b, beta);
	double denom = p_a_beta + p_b_beta;
	if (denom < 1e-20) return 0.0;
	return p_a_beta / denom;
}

// Calcula el valor de color para el rayo dado (fases 04-05)
// Para fase 5b (ptexp), bounce es el contador de rebotes (comienza en 1)
Color shade(const Ray &r, unsigned short *rand_state, int bounce = 0) {
	double t;
	int id = 0;
	...
	// Si el objeto es un emisor (ke ≠ 0)
	if (obj.ke.x > 0 || obj.ke.y > 0 || obj.ke.z > 0) {
		// Para fase 5b (ptexp), solo devolver emisión si bounce==1 (rayo de cámara)
		if (strcmp(current_mode, "ptexp") == 0) {
			if (bounce == 1) return obj.ke;
			else return Color();
		}
		// Para otros modos, siempre devolver emisión
		return obj.ke;
	}
	...
	// Modo ISSA e ISArea: muestrear TODAS las fuentes (fase 4)
	...
	// Modo ISBRDF: muestrear la BRDF (fase 4)
	...
	// Modo MIS (fase 5a) o PTEXP (fase 5b)
	else if (strcmp(current_mode, "mis") == 0 || strcmp(current_mode, "ptexp") == 0) {
		// Iluminación directa con MIS β=2
		for (int emitter_idx = 0; emitter_idx < *current_emitters_count; emitter_idx++) {
			int light_id = current_emitters[emitter_idx];
			const Sphere &light = current_scene[light_id];

			// Caso 1: Fuente puntual (r=0) — sin MIS, solo determinista
			if (light.r < 1e-6) {
				Vector wi = light.p - x;
				double d = sqrt(wi.dot(wi));
				wi = wi * (1.0 / d);
			...
			// Fuentes esféricas: MIS con β=2
			else {
				// Término 1: Muestra de luz por ángulo sólido
				double xi1_l = erand48(rand_state);
				double xi2_l = erand48(rand_state);
				Vector wi_light;
				double pdf_light;
				sample_solid_angle(light, x, xi1_l, xi2_l, wi_light, pdf_light);

				Color L_light_contrib = Color();
				double ndotl_l = n.dot(wi_light);
				if (ndotl_l > 0.0 && is_finite_pdf(pdf_light)) {
					Ray shadow_ray_l(x + n * 1e-4, wi_light);
					double t_shadow_l;
					int id_shadow_l;
					bool hit_l = intersect(shadow_ray_l, t_shadow_l, id_shadow_l);
					if (hit_l && id_shadow_l == light_id) {
						MaterialEval mat_eval_l = obj.material->eval(n, wo, wi_light);
						// Evaluar pdf_brdf en la dirección muestreada por luz
						double pdf_brdf_l = obj.material->pdf(n, wo, wi_light);
						double w_l = 1.0; // default weight sin MIS
						if (is_finite_pdf(pdf_brdf_l)) {
							w_l = mis_weight(pdf_light, pdf_brdf_l);
						}
						Color c_l = mat_eval_l.fr.mult(light.ke) * (ndotl_l / pdf_light);
						L_light_contrib = c_l * w_l;
					}
				}

				// Término 2: Muestra de BRDF
				MaterialSample sample_b = obj.material->sample(n, wo, rand_state);
				Vector wi_brdf = sample_b.wi;
				double pdf_brdf = sample_b.pdf;
				// Nota: sample_b consume dos muestras aleatorias del RNG internamente

				Color L_brdf_contrib = Color();
				if (is_finite_pdf(pdf_brdf)) {
					double ndotl_b = n.dot(wi_brdf);
					if (ndotl_b > 0.0) {
						Ray wi_ray_b(x + n * 1e-4, wi_brdf);
						double t_wi_b;
						int id_wi_b;
						if (intersect(wi_ray_b, t_wi_b, id_wi_b) && id_wi_b == light_id) {
							MaterialEval mat_eval_b = obj.material->eval(n, wo, wi_brdf);
							// Evaluar pdf_luz en la dirección muestreada por BRDF
							Vector x_to_light = light.p - x;
							double dist_to_light = sqrt(x_to_light.dot(x_to_light));
							double cos_theta_max_b = sqrt(std::max(0.0, 1.0 - (light.r / dist_to_light) * (light.r / dist_to_light)));
							double pdf_light_b = 1.0 / (2.0 * M_PI * std::max(1e-6, 1.0 - cos_theta_max_b));

							double w_b = 1.0; // default weight sin MIS
							if (is_finite_pdf(pdf_light_b)) {
								w_b = mis_weight(pdf_brdf, pdf_light_b);
							}
							Color c_b = mat_eval_b.fr.mult(light.ke) * (ndotl_b / pdf_brdf);
							L_brdf_contrib = c_b * w_b;
						}
					}
				}

				// Acumular contribución MIS
				L_total = L_total + L_light_contrib + L_brdf_contrib;
			}
		}

		// Para PTEXP: rebote indirecto con recursión
		if (strcmp(current_mode, "ptexp") == 0) {
			if (bounce < ptexp_max_bounce) {
				// Muestrear BRDF para rebote indirecto
				MaterialSample sample_indirect = obj.material->sample(n, wo, rand_state);
				Vector wi_indirect = sample_indirect.wi;
				double pdf_indirect = sample_indirect.pdf;

				if (is_finite_pdf(pdf_indirect)) {
					double ndotl_ind = n.dot(wi_indirect);
					if (ndotl_ind > 0.0) {
						Ray indirect_ray(x + n * 1e-4, wi_indirect);
						Color L_indirect = shade(indirect_ray, rand_state, bounce + 1);

						// Actualizar L_indirect para valores válidos
						if ((L_indirect.x >= 0.0 && L_indirect.x < 1e20) &&
						    (L_indirect.y >= 0.0 && L_indirect.y < 1e20) &&
						    (L_indirect.z >= 0.0 && L_indirect.z < 1e20)) {
							MaterialEval mat_eval_ind = obj.material->eval(n, wo, wi_indirect);
							Color contrib_ind = mat_eval_ind.fr.mult(L_indirect) * (ndotl_ind / pdf_indirect);
							L_total = L_total + contrib_ind;
						}
					}
				}
			}
	...
	// Validar resultado final para evitar NaN/infinito
	if (!(L_total.x >= 0.0 && L_total.x < 1e20)) L_total.x = 0.0;
	if (!(L_total.y >= 0.0 && L_total.y < 1e20)) L_total.y = 0.0;
	if (!(L_total.z >= 0.0 && L_total.z < 1e20)) L_total.z = 0.0;
...
int main(int argc, char *argv[]) {
	...
	// Parsear argumentos
	// Para fase 4: <modo> <spp> <escena>
	// Para fase 5a: mis <spp> <escena>
	// Para fase 5b: ptexp <spp> <escena> [B]
	if (argc < 3) {
		fprintf(stderr, "Usage: %s <modo> <spp> [escena] [B]\n", argv[0]);
		fprintf(stderr, "  Fase 4: %s <issa|isarea|isbrdf> <spp> <escena>\n", argv[0]);
		fprintf(stderr, "  Fase 5a: %s mis <spp> <escena>\n", argv[0]);
		fprintf(stderr, "  Fase 5b: %s ptexp <spp> <escena> [B]\n", argv[0]);
		return 1;
	}
	...
	// Detectar si es fase 5b (ptexp)
	if (strcmp(current_mode, "ptexp") == 0) {
		if (argc < 4) {
			fprintf(stderr, "Usage: %s ptexp <spp> <escena> [B]\n", argv[0]);
			return 1;
		}
		const char *scene_name = argv[3];
		if (argc >= 5) {
			ptexp_max_bounce = atoi(argv[4]);
		}

		// Configurar escena para ptexp
		if (strcmp(scene_name, "areal") == 0) {
			current_scene = scene_ptexp_areal;
			current_scene_count = scene_ptexp_areal_count;
			current_emitters = emitters_ptexp_areal;
			current_emitters_count = &emitters_ptexp_areal_count;
		} else if (strcmp(scene_name, "point") == 0) {
			current_scene = scene_ptexp_point;
			current_scene_count = scene_ptexp_point_count;
			current_emitters = emitters_ptexp_point;
			current_emitters_count = &emitters_ptexp_point_count;
		} else if (strcmp(scene_name, "multil") == 0) {
			current_scene = scene_ptexp_multil;
			current_scene_count = scene_ptexp_multil_count;
			current_emitters = emitters_ptexp_multil;
			current_emitters_count = &emitters_ptexp_multil_count;
		} else {
			fprintf(stderr, "Escena desconocida: %s (use areal, point o multil)\n", scene_name);
			return 1;
		}
	} else {
		// Fase 4 o 5a
		if (argc < 4) {
			...
			fprintf(stderr, "  modo: issa, isarea, isbrdf, mis\n");
	...
	for(int y = 0; y < h; y++)
	{
		...
		for(int x = 0; x < w; x++ ) {
			...
			// RNG determinista por fila (erand48 con semilla derivada del índice de fila)
			unsigned short rand_state[3];
			if (strcmp(current_mode, "ptexp") == 0) {
				// Para ptexp: RNG determinista por fila
				unsigned int row_seed = 5489 ^ y;
				rand_state[0] = (row_seed >> 16) & 0xFFFF;
				rand_state[1] = row_seed & 0xFFFF;
				rand_state[2] = (row_seed >> 8) ^ 5489;
			} else {
				// Para otros modos: RNG determinista por píxel
			...
			for (int s = 0; s < spp; s++) {
				// Para ptexp (fase 5b), pasar bounce=1 inicial
				Color sample;
				if (strcmp(current_mode, "ptexp") == 0) {
					sample = shade( Ray(camera.o, cameraRayDir), rand_state, 1 );
				} else {
					sample = shade( Ray(camera.o, cameraRayDir), rand_state );
				}
