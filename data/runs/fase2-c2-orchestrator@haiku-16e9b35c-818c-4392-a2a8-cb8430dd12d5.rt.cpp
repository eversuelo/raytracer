...
PointLight pointlight(Point(0, 24.3, 0), Color(4000, 4000, 4000));

// Array de esferas sin el emisor (para modo point)
Sphere spheres_no_emitter[] = {
	Sphere(1e5,  Point(-1e5 - 49, 0, 0),   Color(.75, .25, .25), Color(0, 0, 0)), // pared izq
	Sphere(1e5,  Point(1e5 + 49, 0, 0),    Color(.25, .25, .75), Color(0, 0, 0)), // pared der
	Sphere(1e5,  Point(0, 0, -1e5 - 81.6), Color(.25, .75, .25), Color(0, 0, 0)), // pared detras
	Sphere(1e5,  Point(0, -1e5 - 40.8, 0), Color(.25, .75, .75), Color(0, 0, 0)), // suelo
...
// Intersección para spheres_no_emitter (fase 02, modo point)
inline bool intersect_no_emitter(const Ray &r, double &t, int &id) {
	t = 1e20;
	bool found = false;
	for (int i = 0; i < 7; i++) {
		double hit = spheres_no_emitter[i].intersect(r);
		if (hit > 0.0 && hit < t) {
			t = hit;
			id = i;
			found = true;
...
// Fase 02: Shading determinista con fuente puntual (sin Monte Carlo)
Color shade_pointlight(const Ray &r) {
	double t;
	int id = 0;
	if (!intersect_no_emitter(r, t, id))
		return Color();

	const Sphere &obj = spheres_no_emitter[id];
	Point x = r.o + r.d * t;
	Vector n = (x - obj.p);
	n.normalize();

	// Calcular dirección hacia la luz
	Vector w = pointlight.p - x;
	double r_dist = sqrt(w.dot(w)); // distancia a la luz
	w = w * (1.0 / r_dist); // normalizar

	// Calcular coseno entre normal y dirección hacia la luz
	double costheta = n.dot(w);
	if (costheta <= 0.0)
		return Color(); // cara en sombra

	// Lanzar rayo de sombra desde x hacia la luz
	Ray shadowRay(x + n * 1e-4, w);
	double t_shadow;
	int id_shadow = 0;
	if (intersect_no_emitter(shadowRay, t_shadow, id_shadow)) {
		// Si hay una intersección con t < r - epsilon, está en sombra
		if (t_shadow < r_dist - 1e-4)
			return Color(); // contribución es 0
	}

	// La luz es visible: calcular L = (albedo/π) ⊙ I · cosθ / r²
	Vector fr = obj.c * (1.0 / M_PI);
	double r_squared = r_dist * r_dist;
	Color contrib = pointlight.I.mult(fr) * (costheta / r_squared);
...
int main(int argc, char *argv[]) {
	...
	for(int y = 0; y < h; y++)
	{
		...
		for(int x = 0; x < w; x++) {
			...
			if (use_pointlight) {
				// Modo puntual: 1 rayo por píxel, sin jitter, determinista
				Color sample = shade_pointlight(camRay);
				pixelValue = sample;
			} else {
