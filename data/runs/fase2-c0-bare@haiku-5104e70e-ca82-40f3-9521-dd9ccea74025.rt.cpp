...
// Representación dual para fase 1 (emisor esférico) y fase 2 (fuente puntual).
// En fase 2, la esfera en índice 7 se reemplaza con radio=0 (ningún rayo la intersecta)
// y ke=(4000,4000,4000) representando la intensidad radiante.
Sphere spheres[] = {
	...
	Sphere(10.5, Point(0, 24.3, 0),        Color(1, 1, 1), Color(10, 10, 10)) // emisor (fase 1)
};

// sampler seleccionado: "uniformsphere", "uniformhemi", "cosinehemi"
const char *sampler = "cosinehemi";

// Banderas para modo puntual (fase 2)
bool pointLightMode = false;
Point pointLightPos = Point(0, 24.3, 0);
Color pointLightIntensity = Color(4000, 4000, 4000);
...
// Iluminación directa determinista con fuente puntual (fase 2)
// Sin Monte Carlo: 1 rayo por píxel, shading exacto hacia la luz.
Color shadePointLight(const Ray &r) {
	double t;
	int id = 0;
	// determinar que esfera intersecta
	if (!intersect(r, t, id))
		return Color();	// rayo no intersectó objeto

	const Sphere &obj = spheres[id];

	// Si intersecta la fuente puntual (radio ≈ 0), no contribuye
	if (obj.r < 1e-6) {
		return Color();
	}
	...
	// dirección hacia la luz y distancia
	Vector toLight = pointLightPos - x;
	...
	if (costheta <= 0.0) {
		return Color();	// parte trasera, no ilumina
	}

	// rayo de sombra desde x hacia la luz
	Ray shadowRay(x + n * 1e-4, toLight);

	// verificar oclusión: si alguna esfera bloquea antes de llegar a la luz
	bool occluded = false;
	for (int i = 0; i < 8; i++) {
	...
	// Contribución: L = (albedo/π) ⊙ I · cosθ / r²
	double distSq = distToLight * distToLight;
	Color L = obj.c.mult(pointLightIntensity) * (costheta / (M_PI * distSq));
...
int main(int argc, char *argv[]) {
	...
	// Procesar argumentos CLI: detectar modo puntual o sampler/spp
	const char *selectedSampler = "cosinehemi";
	int spp = 32;
	const char *outputFile = "image.ppm";

	if (argc > 1) {
		if (strcmp(argv[1], "point") == 0) {
			// Modo puntual (fase 2): iluminación directa determinista
			pointLightMode = true;
			outputFile = "image-plight.ppm";
			// Reemplazar esfera emisora con fuente puntual (radio 0, ke = I)
			spheres[7] = Sphere(0, Point(0, 24.3, 0), Color(1, 1, 1), Color(4000, 4000, 4000));
			spp = 1;	// 1 rayo por píxel, sin jitter
		} else {
			// Modo Monte Carlo (fase 1): sampler seleccionado
			selectedSampler = argv[1];
		}
	}

	if (argc > 2 && !pointLightMode) {
		spp = atoi(argv[2]);
	}
	...
	for(int y = 0; y < h; y++)
	{
		...
		for(int x = 0; x < w; x++ ) {
			...
			if (pointLightMode) {
				// Modo puntual: 1 rayo determinista sin muestreo
				pixelValue = shadePointLight(cameraRay);
			} else {
				// Modo Monte Carlo: promediar spp muestras
				for (int s = 0; s < spp; s++) {
