...
// modo de depuración seleccionado en main() a partir de argv
enum RenderMode { MODE_NORMAL, MODE_DISTANCE };
RenderMode renderMode = MODE_NORMAL;

// t extremos visibles de la escena (spec RF-3, modo distancia)
const double T_NEAR = 144.676098;
const double T_FAR  = 304.110891;
...
Color shade(const Ray &r) {
	...
	Color colorValue;
	if (renderMode == MODE_DISTANCE) {
		double g = (t - T_NEAR) / (T_FAR - T_NEAR);
...
int main(int argc, char *argv[]) {
	...
	// modo de depuración: ./rt normal (default) | ./rt distance
	if (argc > 1 && strcmp(argv[1], "distance") == 0)
		renderMode = MODE_DISTANCE;
	else
		renderMode = MODE_NORMAL;
